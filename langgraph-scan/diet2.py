...
def extract_location_fields(
    ...
    context_region = get(physical, ["region"], {}) or {}
...
def strip_trailing_doc_link(message_text: str) -> str:
    # drop trailing sentences like "See https://..." that are not core findings
    return re.sub(r"\s*See https?://\S+(?:\s+.*)?$", "", message_text or "", flags=re.IGNORECASE)

def message_with_source_placeholders(message_text: str) -> str:
    # "[label](n)" -> "<source#N>"
    cleaned = strip_trailing_doc_link(message_text or "")
    return ANCHOR_RE.sub(lambda m: f"<source#{m.group(2)}>", cleaned)
